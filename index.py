...
class TelegramClient:
    ...
    async def _post(method, payload):
        ...
        try:
            async with Resources.client_session.post(Config.TG_API_BASE + method, json=payload, timeout=5) as resp:
                pass 
        except Exception as e:
    ...
    async def _process_message(msg):
        ...
        if not order:
            if 'contact' in msg or (text and re.match(r'^\+?\d{10,15}$', re.sub(r'[^\d+]', '', text))):
                 ...
                 return
            await TelegramClient.send_message(chat_id, f"🛍 Чтобы сделать заказ, перейдите в наш магазин. По всем вопросам: {Config.ADMIN_SUPPORT_USERNAME}")
            return
...
@app.get("/")
async def root(): return "FastAPI Bot Running"
...
async def init_auth(request: Request):
    try:
        ...
        if not Resources.db_pool:
             return Response(content="DB Not Ready", status_code=500)
